...
# Número de episodios sin mejora para reiniciar epsilon
epsilon_reset_threshold = 20000
...
for episode in range(n_episodes):
    ...
    # Evaluar desempeño para posible reinicio de epsilon
    if (episode + 1) % epsilon_reset_threshold == 0:
        promedio_reward = np.mean(rewards[-epsilon_reset_threshold:])
        if promedio_reward < best_avg_reward:
            epsilon = epsilon_max*0.1  # Reiniciar epsilon si no hay mejora
            print(
                f"Reinicio de epsilon a {epsilon_max}. No hubo mejora en los últimos {epsilon_reset_threshold} episodios.")
        else:
